...
int main(int argc, char const *argv[]){
  ...
  server = new DatagramServer(MULTICAST_PORT, &mcast_hints, multitcast, local, UNICAST_PORT, &ucast_hints);
  server->display_address();
  //server->enable_polling(server->get_socket_fd());
  // server->register_service(2222);
  // server->register_service(5555);
  // server->display_services();
